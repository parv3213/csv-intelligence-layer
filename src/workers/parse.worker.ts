...
import { ingestions, decisionLogs } from '../db/schema.js';
import { parseCSV, toInferenceInput } from '../services/csv-parser.js';
import { redis, QUEUE_NAMES, inferQueue, type ParseJobData } from './queues.js';
...
parseWorker.on('completed', (job) => {
  log.info({ jobId: job.id, ingestionId: job.data.ingestionId }, 'Parse job completed');
});
